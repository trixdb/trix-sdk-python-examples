...
from trix.exceptions import RateLimitError, ServerError, TrixError
...
def example_simple_retry():
    with Trix.from_env() as client:
        memory = retry_with_backoff(lambda: client.memories.create(content="Test"))
...
async def example_async_retry():
    async with AsyncTrix.from_env() as client:
        memory = await async_retry_with_backoff(lambda: client.memories.create(content="Test"))
        print(f"Created: {memory.id}")
