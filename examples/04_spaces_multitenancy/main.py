...
def main() -> None:
    ...
    with Trix.from_env() as client:
        ...
        results_a = client.search.query(query="tenant data", space_id=tenant_a.id, limit=10)
        print(f"   Tenant A results: {len(results_a.data)}")

        # Search only in Tenant B's space
        results_b = client.search.query(query="tenant data", space_id=tenant_b.id, limit=10)
