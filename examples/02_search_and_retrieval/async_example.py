...
async def main() -> None:
    ...
    async with AsyncTrix.from_env() as client:
        ...
        search_tasks = [client.search.query(query=q, limit=3) for q in queries]
        ...
        similar_tasks = [client.search.similar(memory_id=m.id, limit=2) for m in memories]
        ...
        await client.search.embed_all(batch_size=100)
        print("   ✓ Processed all memories in batches")
        ...
        topic_results = await client.search.by_topic(topic="software development", limit=5)
