...
def test_not_found_error():
    ...
    respx.get("https://api.trixdb.com/v1/memories/nonexistent").mock(
        return_value=Response(
            404, json={"error": {"message": "Memory not found", "code": "not_found"}}
        )
    )
...
def test_rate_limit_error():
    ...
    respx.get("https://api.trixdb.com/v1/memories").mock(
        return_value=Response(
            429, json={"error": {"message": "Rate limit exceeded"}}, headers={"Retry-After": "5"}
        )
    )
...
async def test_async_not_found_error():
    ...
    respx.get("https://api.trixdb.com/v1/memories/nonexistent").mock(
        return_value=Response(404, json={"error": {"message": "Memory not found"}})
    )
