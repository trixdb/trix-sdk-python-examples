...
from trix.exceptions import NotFoundError, TrixError
...
async def demonstrate_concurrent_with_semaphore():
    ...
    async with AsyncTrix.from_env() as client:
        ...
        memories = await asyncio.gather(
            *[client.memories.create(content=f"Concurrent test {i}") for i in range(10)]
        )
...
async def demonstrate_async_iteration():
    ...
    async with AsyncTrix.from_env() as client:
        # Create test data
        memories = await asyncio.gather(
            *[client.memories.create(content=f"Async iter test {i}") for i in range(5)]
        )
    ...
    async with AsyncTrix.from_env() as client:
        try:
            async with asyncio.timeout(5.0):  # 5 second timeout
                await client.memories.list(limit=1)
...
async def demonstrate_gather_with_exceptions():
    ...
    async with AsyncTrix.from_env() as client:
        # Create test data
        memories = await asyncio.gather(
            *[client.memories.create(content=f"Gather test {i}") for i in range(3)]
        )
